...
type PlusMinus = '+' | '-';
type Percent = '%';
type MatchPefix<T> = T extends `${infer Prefix}${infer Rest}` ? Prefix extends PlusMinus ? Prefix : '' : '';
type MatchSuffix<T> = T extends `${infer Rest}${Percent}` ? Percent : '';
type MatchValue<T> = T extends `${MatchPefix<T>}${infer V}${MatchSuffix<T>}` ? V : '';
type PercentageParser<A extends string> = [MatchPefix<A>, MatchValue<A>, MatchSuffix<A>];
